...
(function($){
	...
	/*----------------------------------------------------*/
		...
		$('.listing-widget').on('click', 'a.book-now', function(e){
			...
			try {
				if ( freeplaces > 0 ) 
				{
						...
						//second calendar
						var endDataSql = moment( $('#date-picker1').data('daterangepicker').endDate, ["MM/DD/YYYY"]).format("YYYY-MM-DD");
						//second calendar
						var ajax_data = {
		...
		$('#date-picker1').daterangepicker({
			...
			"opens": "left",
			// checking attribute listing type and set type of calendar
			singleDatePicker: ( $('#date-picker1').data('listing_type') == 'rental' ? false : true ), 
			timePicker: false,
			minDate: moment().subtract(0, 'days'),
			minSpan : { days:  $('#date-picker1').data('minspan') },
			startDate : Cookies.get('truelysell_rental_startdate'),
			endDate : Cookies.get('truelysell_rental_enddate'),
			locale: {
				format: wordpress_date_format.date,
		...
		function check_booking() 
		{
			...
			Cookies.set('truelysell_rental_enddate',  $('#date-picker1').data('daterangepicker').endDate.format(wordpress_date_format.date));
			...
			console.log($('#date-picker1').data('daterangepicker').endDataSql);
				...
				success: function(data){
					...
					if (data.success == true && ( ! $(".time-picker").length || is_open != false ) ) {
					   if ( data.data.free_places > 0) {
								...
								$('.booking-estimated-da span').html(data.data.da);
								$('.booking-estimated-da').data('da',data.data.da);
								$('.booking-estimated-da1 span').html(data.data.da1);
								$('.booking-estimated-da1').data('da1',data.data.da1);
		...
		function update_booking_widget () 
		{
			...
			$.ajax({
				...
				complete: function() {
					// Handle after AJAX call is complete
					$('a.book-now').removeClass('loading');
					$('a.book-now-notloggedin').removeClass('loading');
				}
				//second calendar
		...
		//second calendar
		$( '#date-picker1' ).on( 'apply.daterangepicker', update_booking_widget );
		...
		// //second calendar
		// $( '#date-picker1' ).on( 'apply.daterangepicker', check_booking );
		// $( '#date-picker1' ).on( 'cancel.daterangepicker', check_booking );
		// //second calendar
